...
export function initSourceEditModel(config, onSaveCallback) {
    ...
    if (!sourceEditModel || !sourceEditForm || !sourceEditTitle || !sourceEditIdInput ||
        ...
        console.error("[SourceEditModel] 初始化失败：一个或多个必需的 DOM 元素未找到。请检查配置中的 ID:", config);
        return;
    }
...
export function openSourceEditModel(sourceToEdit = null) {
    if (!sourceEditModel || !sourceEditForm) {
        console.error("[SourceEditModel] openSourceEditModel 失败：弹窗或表单元素未初始化");
        return;
    }
    const mode = sourceToEdit ? '编辑' : '添加';
    console.log(`[SourceEditModel] 开始打开数据源编辑弹窗 (${mode}模式)`);
    ...
    if (sourceToEdit) {
        ...
        console.log(`[SourceEditModel] 填充表单以编辑数据源: ${sourceToEdit.name} (ID: ${sourceToEdit.id})`);
        ...
        // Adding new source
        console.log('[SourceEditModel] 准备表单以添加新数据源');
    ...
    console.log('[SourceEditModel] 数据源编辑弹窗已打开');
    try {
        sourceEditNameInput.focus(); // Focus the name field
        console.debug('[SourceEditModel] 已聚焦名称输入框');
    } catch (focusError) {
         // Task 1: Error Logging
        console.error('[SourceEditModel] 聚焦名称输入框时出错:', focusError);
    }
}

/** Closes the source edit Model. */
export function closeSourceEditModel() {
    console.log('[SourceEditModel] 开始关闭数据源编辑弹窗');
    if (sourceEditModel) {
        sourceEditModel.classList.remove('active');
         console.log('[SourceEditModel] 数据源编辑弹窗已关闭');
    } else {
        console.warn('[SourceEditModel] closeSourceEditModel 调用时弹窗元素未初始化');
    }
}
...
function handleSourceTypeChange() {
    if (!sourceEditTypeSelect || !sourceEditLocalFields || !sourceEditWebdavFields || !sourceEditPathInput || !sourceEditUrlInput) {
         console.error("[SourceEditModel] handleSourceTypeChange 失败：一个或多个必需的表单字段元素未初始化");
        return;
    }
    ...
    if (selectedType === 'local') {
        ...
        console.warn(`[SourceEditModel] 未知的数据源类型被选中: ${selectedType}`);
        sourceEditLocalFields.style.display = 'none';
        sourceEditWebdavFields.style.display = 'none';
        sourceEditPathInput.required = false;
        sourceEditUrlInput.required = false;
...
async function handleBrowseFolder() {
    ...
    if (!sourceEditBrowseBtn || !sourceEditPathInput) {
         console.error("[SourceEditModel] handleBrowseFolder 失败：浏览按钮或路径输入框未初始化");
        return;
    }
    console.log("[SourceEditModel] 开始处理浏览文件夹操作");
    sourceEditBrowseBtn.disabled = true; // Disable button during operation
    clearFeedback(sourceEditFeedbackEl);

    try {
        console.log("[SourceEditModel] 调用 API 打开文件夹选择对话框");
        const selectedPath = await window.api.openFolderDialog();
        if (selectedPath) {
            console.log(`[SourceEditModel] 用户选择了文件夹: ${selectedPath}`);
            sourceEditPathInput.value = selectedPath;
        } else {
             console.log("[SourceEditModel] 用户取消了文件夹选择");
        }
    } catch (error) {
         // Task 1: Error Logging
        console.error("[SourceEditModel] 打开文件夹对话框失败:", error.message, error.stack, error);
        ...
        console.log("[SourceEditModel] 浏览文件夹操作完成");
    }
}
...
function handleSourceEditFormSubmit(event) {
    ...
    if (!_onSaveCallback) {
         // Task 1: Error Logging
        console.error("[SourceEditModel] 保存失败：_onSaveCallback 未定义");
        showFeedback(sourceEditFeedbackEl, '内部错误：保存回调未设置', 'error'); // Provide generic user feedback
        return;
    }

    clearFeedback(sourceEditFeedbackEl);
    console.log('[SourceEditModel] 开始处理表单提交和验证');
    ...
    if (!sourceName) {
        ...
        console.warn(`[SourceEditModel] 验证失败: ${errorMsg}`);
    ...
    console.debug(`[SourceEditModel] 基本数据: ID=${newSourceData.id}, Name=${newSourceData.name}, Type=${newSourceData.type}`);
    ...
    if (sourceType === 'local') {
        ...
        try {
            ...
            console.warn(`[SourceEditModel] 验证失败 (webdav): ${errorMsg} - ${urlValue}`);
            showFeedback(sourceEditFeedbackEl, errorMsg, 'error');
            sourceEditUrlInput.focus();
            return;
        }
        ...
        console.debug(`[SourceEditModel] WebDAV URL: ${urlValue}, Username: ${newSourceData.username}`);
    } else {
         // Task 1: Error Logging (Should not happen)
        const errorMsg = t('settings.validation.typeRequired');
        console.error(`[SourceEditModel] 验证失败：无效的数据源类型 "${sourceType}"`);
        showFeedback(sourceEditFeedbackEl, errorMsg, 'error');
        return; // Should not happen if dropdown is set up correctly
    }

    // --- Call the Save Callback ---
    console.log(`[SourceEditModel] 验证通过，调用保存回调函数，数据:`, newSourceData);
