...
async _recursiveListAllFiles(resolvedCurrentPath, currentShowSubdirectory) {
  ...
  async readModelDetail(relativePath, modelFileName, sourceId) { // Added modelFileName and sourceId
    const startTime = Date.now();
    await this.ensureInitialized();
    const currentSourceId = sourceId || this.config.id; // Use provided sourceId or fallback to config

    if (!relativePath) {
      log.warn(`[WebDavDataSource][${currentSourceId}] readModelDetail 调用时 relativePath (jsonPath) 为空`);
      return {};
    }
    if (!modelFileName) {
      log.warn(`[WebDavDataSource][${currentSourceId}] readModelDetail 调用时 modelFileName 为空 (jsonPath: ${relativePath})`);
      return {};
    }

    const resolvedJsonPath = this._resolvePath(relativePath);
    log.debug(`[WebDavDataSource][${currentSourceId}] 开始读取模型详情: resolvedJsonPath=${resolvedJsonPath} (relativeJsonPath: ${relativePath}), modelFileName=${modelFileName}, sourceId=${currentSourceId}`);

    try {
      const jsonContentString = await this.client.getFileContents(resolvedJsonPath, { format: 'text' });

      // Construct modelFileInfo
      const modelFileBaseName = path.posix.basename(modelFileName, path.posix.extname(modelFileName));
      // Assume model file is in the same directory as the JSON file
      const modelFileRelativePath = path.posix.join(path.posix.dirname(relativePath), modelFileName);

      const modelFileInfo = {
        name: modelFileBaseName,
        file: modelFileRelativePath, // Relative path to model file on WebDAV
        jsonPath: relativePath,     // Relative path to JSON file on WebDAV
        ext: path.posix.extname(modelFileName), // Added ext property
      };

      // Call parseModelDetailFromJsonContent with jsonContent, sourceIdentifier (sourceId), and modelFileInfo
      const detail = parseModelDetailFromJsonContent(jsonContentString, currentSourceId, modelFileInfo);
      const duration = Date.now() - startTime;
...
async writeModelJson(relativePath, dataToWrite) { // dataToWrite is now an object (modelJsonInfo)
    const startTime = Date.now();
    await this.ensureInitialized();
    ...
    try {
      ...
      // Serialize the dataToWrite object to a formatted JSON string
      const jsonStringToWrite = JSON.stringify(dataToWrite, null, 2);
      await this.client.putFileContents(resolvedPath, jsonStringToWrite, { overwrite: true });
      const duration = Date.now() - startTime;
