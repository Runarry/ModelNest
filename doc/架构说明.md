...
*   **日志格式与内容**:
    ...
    *   **渲染进程**: 通过 `window.api.logMessage(level, message, ...args)` 发送日志。主进程接收后会自动添加 `[Renderer]` 前缀。渲染进程内部代码也应使用类似 `[UI]`, `[i18n]`, `[Component:MainView]` 等前缀区分来源。
    *   **错误日志**:
...
*   **代码风格**: （建议补充，例如：遵循特定的 Linter 规则，命名约定，注释规范等）
