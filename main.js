...
if (!gotTheLock) {
  ...
  log.info('[SingleInstance] 应用已在运行，退出当前实例');
  app.quit();
} else {
  // 监听第二个实例启动事件
  app.on('second-instance', (event, commandLine, workingDirectory) => {
    // 当用户尝试启动第二个实例时，应该显示并激活已存在的主窗口
    log.info('[SingleInstance] 检测到第二个实例启动，激活现有窗口');
    ...
    if (mainWindow) {
      ...
      mainWindow.setAlwaysOnTop(false); // 短暂置顶后恢复正常层级
    }
  });

  // 只有获得单实例锁的进程才继续执行应用逻辑
